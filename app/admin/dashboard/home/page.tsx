...
import { HomeContent, AwardItem } from "@/types/content";
import toast from "react-hot-toast";
import { defaultAwards } from "@/lib/defaults";
import { Plus, Trash, Award, Image as ImageIcon } from "lucide-react";
...
export default function HomeEditor() {
    ...
    async function load() {
      ...
      if (dbData) {
        setData({
          ...
          awardsHeading: dbData.awardsHeading || defaults.awardsHeading,
          awards: dbData.awards || defaults.awards || defaultAwards,
  ...
  const handleUpdateAward = (index: number, field: keyof AwardItem, val: string) => {
    setData((prev) => {
  ...
  return (
    <div className="space-y-10 max-w-4xl pb-16">
      ...
      {/* AWARDS & WINS SECTION EDITOR */}
      <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm space-y-6">
        <div className="flex justify-between items-center border-b pb-2">
          <div>
            <h2 className="text-lg font-bold text-brand-dark flex items-center gap-2">
              <Award className="w-5 h-5 text-[#C8A400]" /> Awards &amp; Wins Section
            </h2>
            <p className="text-xs text-gray-500 mt-0.5">
              Manage awards displayed in the horizontal auto-scrolling marquee carousel on the home page.
            </p>
          </div>
          <button
            type="button"
        ...
        <div>
          ...
          <input
            type="text"
            value={data.awardsHeading || "Our latest awards and wins:"}
            onChange={(e) => setData({ ...data, awardsHeading: e.target.value })}
            className="w-full px-3 py-2 border rounded focus:outline-none focus:border-brand-green text-sm bg-white text-brand-dark font-serif"
            placeholder="Our latest awards and wins:"
          />
        </div>
        ...
        <div className="space-y-4">
          {(data.awards || []).map((award, idx) => (
            <div
              key={idx}
              className="p-4 bg-gray-50 border rounded-lg flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between"
            >
              <div className="w-20 h-20 rounded border bg-gray-200 flex items-center justify-center shrink-0 overflow-hidden relative">
                {award.imageUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={award.imageUrl}
                    alt={award.title || "Award Preview"}
                    className="max-h-full max-w-full object-contain"
                  />
                ) : (
              ...
              <div className="flex-grow grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleUpdateAward(idx, "imageUrl", e.target.value)}
                    placeholder="https://..."
                    className="w-full px-2.5 py-1.5 border rounded focus:outline-none focus:border-brand-green text-xs bg-white text-brand-dark"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">
                  ...
                  <input
                    ...
                    placeholder="Butcher of the Year 2026"
                    className="w-full px-2.5 py-1.5 border rounded focus:outline-none focus:border-brand-green text-xs bg-white text-brand-dark font-semibold"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="p-2 text-brand-red hover:bg-brand-red/10 rounded shrink-0 self-center"
                title="Remove Award"
              >
